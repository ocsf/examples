# Event Dossier: Okta system log Authentication

### An Okta system log for 
- **Description**: Translates a `user.session.start` log to OCSF.
- **Event References**:
...
|`logon_type/logon_type_id`|`transaction.type`|
|`actor.user.type/actor.user.type_id/user.type/user.type_uid`|`actor.type`|
